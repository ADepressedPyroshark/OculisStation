...
/datum/team/miscreants/New(forced_max = 0, custom_name, custom_flavor, custom_objective, custom_ooc)
	...
	if(forced_max > 0)
		...
		max_miscreants = clamp(round(GLOB.alive_player_list.len * 0.1, 1), 2, 4) //if no value is specified, make the team size 1/5th of the current pop, but never fewer than 2 nor more than 8
